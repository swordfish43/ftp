...
class FtpWindow(QtGui.QDialog):
    def __init__(self, parent=None):
        ...
        self.ftpServerLineEdit = QtGui.QLineEdit('ftp.debian.org')
        ftpServerLabel.setBuddy(self.ftpServerLineEdit)

        self.statusLabel = QtGui.QLabel(u"Введите адрес FTP сервера.")

        self.fileList = QtGui.QTreeWidget()
        self.fileList.setEnabled(False)
        self.fileList.setRootIsDecorated(False)
        self.fileList.setHeaderLabels((u"Имя", u"Размер", u"Владелец", u"Группа", u"Дата"))
        self.fileList.header().setStretchLastSection(False)

        self.connectButton = QtGui.QPushButton(u"Соединение")
        ...
        self.downloadButton = QtGui.QPushButton(u"Скачать")
        self.downloadButton.setEnabled(False)

        self.quitButton = QtGui.QPushButton(u"Выход")
    ...
    def downloadFile(self):
        ...
        if QtCore.QFile.exists(fileName):
            QtGui.QMessageBox.information(self, "FTP",
                    u"Файл с названием %s уже существует в текущей "
                    u"директории." % fileName)
            return

        self.outFile = QtCore.QFile(fileName)
        if not self.outFile.open(QtCore.QIODevice.WriteOnly):
            QtGui.QMessageBox.information(self, "FTP",
                    u"Невозможно сохранить файл %s: %s." % (fileName, self.outFile.errorString()))
            self.outFile = None
            return

        self.ftp.get(self.fileList.currentItem().text(0), self.outFile)

        self.progressDialog.setLabelText(u"Скачиваю %s..." % fileName)
        self.downloadButton.setEnabled(False)
    ...
    def ftpCommandFinished(self, _, error):
        ...
        if self.ftp.currentCommand() == QtNetwork.QFtp.ConnectToHost:
            if error:
                QtGui.QMessageBox.information(self, "FTP",
                        u"Невозможно соединиться с FTP серовером по адресу %s. Пожалуйста "
                        u"проверьте правильно ли введен адрес." % self.ftpServerLineEdit.text())
                self.connectOrDisconnect()
                return

            self.statusLabel.setText(u"Выполнен вход в %s." % self.ftpServerLineEdit.text())
        ...
        if self.ftp.currentCommand() == QtNetwork.QFtp.Get:
            if error:
                self.statusLabel.setText(u"Загрузка %s отменена." % self.outFile.fileName())
                self.outFile.close()
                self.outFile.remove()
            else:
                self.statusLabel.setText(u"Файл %s скачан в текущую директорию." % self.outFile.fileName())
                self.outFile.close()
